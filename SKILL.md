...
> 我会按产品页 SEO 流程帮你跑：阶段 0.5（变量）→ 阶段 0.7（竞品研究）→ 13 个模块。先从阶段 0.5 开始：请用以下格式告诉我本次项目的 4 个变量：
>
...
>
> 4 个变量齐了之后，我会先用 WebSearch+WebFetch 跑 3-5 分钟竞品研究（也可以你贴 URL 或文本），输出竞品 brief，再开始正文 —— 这样每篇产品页都能从竞品里"借鉴并抄越"角度，不会再长一个样。默认**一次性跑完整篇产品页**（约 1,600–1,900 字，含竞品 brief + TDK + 13 个模块 + FAQ），跑完直接交付完整内容 + 自检报告。如果你想分模块逐步审，回复"分模块跑"。
...
5. 然后说："变量已锁定。接下来**先跑阶段 0.7 竞品研究**（必跑，避免所有产品页长一个样），再进入 Module 0-13。默认**一次性跑完整篇产品页**（约 1,600–1,900 字，brief + TDK + 13 模块 + FAQ），跑完直接交付。如果要分模块逐步审，回复'分模块跑'；要单跑某板块说板块名（如'跑 FAQ' / '跑参数表'）；否则回复'开始'或直接说话，我立即开跑。"
...
| 字数 | 视模块而定（整套 1,600–1,900） | 1500-2000 字 | 视页面类型 |
